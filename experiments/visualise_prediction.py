"""
visualise_prediction.py  —  Make flood map figures from a completed run

Usage (from the terramind root, in an interactive HPC session):
    pixi run python experiments/visualise_prediction.py <run_dir> [--n 5]

Arguments:
    run_dir   The timestamped run folder, e.g. output/sen1floods11_base_20260407_2038
...
How to start an interactive session for this:
    ...
    pixi run python experiments/visualise_prediction.py output/sen1floods11_terramind_base_20260408_1635
...
def find_source_files(pred_path):
    ...
    scene = re.sub(r"_S[12]Hand.*", "", stem)
    if not scene:
        scene = stem

    # Common dataset root locations (relative to terramind root)
    s2_dir   = "sen1floods11_v1.1/data/S2L1CHand"
    lbl_dir  = "sen1floods11_v1.1/data/LabelHand"

    # Find S2L1C image matching this scene
    image_path = None
    for pattern in [f"{scene}_S2Hand.tif", f"{scene}*_S2Hand.tif"]:
        matches = glob.glob(os.path.join(s2_dir, pattern))
        if matches:
    ...
    label_path = None
    for pattern in [f"{scene}_LabelHand.tif", f"{scene}*_LabelHand.tif"]:
...
def make_figure(pred_path, image_path, label_path, out_path):
    ...
    if image_path and os.path.exists(image_path):
        ...
        axes[0].set_title("S2L1C RGB input", fontsize=11)
    axes[0].axis("off")
    ...
    # Legend
    patches = [mpatches.Patch(color=CLASS_COLOURS[0], label="Others"),
               mpatches.Patch(color=CLASS_COLOURS[1], label="Flood")]
...
def main():
    parser = argparse.ArgumentParser(
        description="Make flood map figures from a completed TerraMind run")
    parser.add_argument("run_dir",
        help="Path to the run folder, e.g. output/sen1floods11_base_20260407_2038")
    parser.add_argument("--n", type=int, default=5,
        help="Number of scenes to plot (default: 5). Use 0 for all scenes.")
