...
ASPA135_CFG = dict(
    ...
    # test_split  = "sample_data/splits/test.txt",
    img_grep    = ".tif",
    label_grep  = ".shp",
...
if DATASET == "burnscars":
    ...
    split_path = ASPA135_CFG["test_split"] # only test split available
    data_root  = ASPA135_CFG["data_root"]
    _img_suffix = ".tif"
    _lbl_suffix = ".shp"
        ...
        if os.path.isfile(candidate_img):
            ...
            if DATASET == "burnscars":
                ...
                candidate_lbl = os.path.join(
                    ASPA135_CFG["label_root"], stem + _lbl_suffix)
            else:
                candidate_lbl = os.path.join(
...
if DATASET == "burnscars":
    ...
    _modality   = ASPA135_CFG["backbone_modality"]
    _bands      = {_modality: ASPA135_CFG["backbone_bands"]}
    _modalities = [_modality]
else:
    _modality   = None
    _bands      = {}
...
if DATASET == "burnscars":
    ...
    CLASS_COLOURS = {
        -1: [0.10, 0.10, 0.10],   # black — No data
         ...
         1: [0.06, 0.75, 0.06],   # green — Vegetation
    }
else:
    CLASS_COLOURS = {
        -1: [0.10, 0.10, 0.10],   # black — No data
...
if DATASET == "burnscars":
    ...
    _vis_means    = ASPA135_CFG["means"]
    _vis_stds     = ASPA135_CFG["stds"]
    _vis_rgb_idx  = ASPA135_CFG["rgb_indices"]
    _vis_modality = ASPA135_CFG["backbone_modality"]
else:
